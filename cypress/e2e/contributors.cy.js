describe("As a public user, I", () => {
      ...
      .then(user => {
        cy.createProjectWithLeader(user, "Project with budget", 100000, 493591124)
          .as("projectId").then(projectId => {
            cy.requestPayment(projectId, 100, ANTHO, { workItems: ["https://github.com/onlydustxyz/starkonquest/pull/68", "https://github.com/onlydustxyz/starkonquest/pull/68"] }).asRegisteredUser(user).data("requestPayment");
            cy.requestPayment(projectId, 100, ANTHO, { workItems: ["https://github.com/onlydustxyz/starkonquest/pull/68"] }).asRegisteredUser(user).data("requestPayment");
            ...
            cy.requestPayment(projectId, 500, ANTHO, { workItems: ["https://github.com/onlydustxyz/starkonquest/pull/68"] }).asRegisteredUser(user).data("requestPayment");
            cy.requestPayment(projectId, 2000, ANTHO, { workItems: ["https://github.com/onlydustxyz/starkonquest/pull/68"] }).asRegisteredUser(user).data("requestPayment");
            cy.requestPayment(projectId, 10000, ANTHO, { workItems: ["https://github.com/onlydustxyz/starkonquest/pull/68"] }).asRegisteredUser(user).data("requestPayment");
          })
      });
  });
  ...
  it("can see the contributors of a project", function () {
    ...
    cy.get("#contributors_table tbody tr:nth-child(1)").within(() => {
        ...
        cy.get("td:nth-child(2)").should("have.text", "13200 $");
