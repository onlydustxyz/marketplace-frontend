...
import { LOCAL_STORAGE_TOKEN_SET_KEY, useTokenSet } from "src/hooks/useTokenSet";
...
const ApolloWrapperWithoutTokenHook: React.FC<PropsWithChildren> = ({ children }) => {
...
const ApolloWrapperWithTokenHook: React.FC<PropsWithChildren> = ({ children }) => {
  const [displayError, setDisplayError] = useState(false);
  const showToaster = useShowToaster();
  const { T } = useIntl();
...
const ApolloWrapper = isFeatureEnabled(FeatureFlags.FIX_TOKEN_RELOAD)
  ? ApolloWrapperWithoutTokenHook
  : ApolloWrapperWithTokenHook;
