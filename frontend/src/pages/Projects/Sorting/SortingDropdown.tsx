...
export default function SortingDropdown({ all, current, onChange }: Props) {
  ...
  return (
    <Listbox
      ...
      onChange={onChange}
      as="div"
      className="h-fit w-fit divide-y divide-greyscale-50/12 rounded-md border border-greyscale-50/12 bg-whiteFakeOpacity-5 font-walsheim text-sm font-medium text-greyscale-50 shadow-heavy"
    >
      ...
      <Listbox.Options className="flex cursor-pointer flex-col divide-y divide-greyscale-50/12 bg-white/2">
        {all.map(value => (
          <Listbox.Option
            key={value}
            value={value}
