...
export const AvailableConversion: FC<AvailableConversion> = ({
  ...
  const { T } = useIntl();
  const tooltipIdProps =
    tooltipId && !withWrapper
      ? {
          "data-tooltip-id": tooltipId,
          "data-tooltip-hidden":
            variant.type === "full" || variant.type === "compact" ? variant.currency === Currency.USD : undefined,
        }
      : {};
  ...
  const currencyArray = useMemo(() => {
    ...
    if (variant.type === "compact" || variant.type === "full") {
      return [
        {
          currency: variant.currency || Currency.USD,
          amount: variant.totalAmount,
          dollar: variant.type === "full" ? variant.dollar : undefined, // don't show USD conversion if currency is USD
        },
      ];
    }
  ...
  return (
    <>
      <div {...tooltipIdProps} className="flex flex-row items-center justify-start gap-1">
        ...
        {variant.type === "full" && variant.dollar && variant.currency !== Currency.USD ? (
