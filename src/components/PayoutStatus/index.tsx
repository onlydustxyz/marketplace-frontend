...
import { PaymentStatus, PaymentStatusREST } from "src/types";
...
export function PayoutStatusREST({
  status,
  isProjectLeaderView = false,
}: {
  status: components["schemas"]["ProjectRewardResponse"]["status"];
  isProjectLeaderView?: boolean;
}) {
  switch (status) {
    case PaymentStatusREST.COMPLETE:
      return CompleteTag();
    case PaymentStatusREST.PROCESSING:
      return ProcessingTag();
    case PaymentStatusREST.PENDING_SIGNUP:
      return PayoutInfoMissingTag(isProjectLeaderView);
    case PaymentStatusREST.PENDING_INVOICE:
