/// Defines the `impl_diesel_mapping_repository` function that implements the Diesel repository for mapping tables.
...
/// Trait to unwrap a passed type parameter as a `TypePath`
trait UnwrapAsPath {
...
/// Returns the entity types associated with the passed `input`
...
/// Returns the id types associated with the passed `input`
...
/// Implements the Diesel repository for mapping tables for the passed input
pub fn impl_diesel_mapping_repository(input: syn::DeriveInput) -> TokenStream {
    ...
    let delete_all_entity1_of_span_name =
        format!("{table_ident}::{delete_all_entity1_of}",);
    let delete_all_entity2_of_span_name =
        format!("{table_ident}::{delete_all_entity2_of}",);
    ...
    let find_all_entity1_of_span_name =
        format!("{table_ident}::{find_all_entity1_of}",);
    let find_all_entity2_of_span_name =
        format!("{table_ident}::{find_all_entity2_of}",);
