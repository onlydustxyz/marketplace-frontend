/// This module contains the implementation of the `Role` enum.
...
pub enum Role {
    ...
    Admin,
    /// The user is a registered user with a Github user ID and a set of lead projects.
    RegisteredUser {
        /// The set of lead projects IDs.
...
impl<'r> FromRequest<'r> for Role {
    ...
    /// Get the role of a given request.
...
/// Get the registered user role from a given request.
