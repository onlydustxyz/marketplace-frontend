...
use common_domain::{GithubIssueNumber, GithubRepoId};
...
#[get("/api/issues/<repository_id>/<issue_number>")]
pub async fn fetch_issue_by_repository_id_issue_number(
	repository_id: i32,
	issue_number: i32,
	github_client_factory: &State<Arc<GithubClientPatFactory>>,
) -> Result<Json<Response>, HttpApiProblem> {
	...
	let issue = github_client_factory
		.github_service()?
		.issue_by_repo_id(repository_id.clone(), issue_number.clone())
		.await
		.map(Into::into)
		.map_err(|e| {
			let error_message = format!(
				"Failed to fetch issue for repository_id {:} and issue_number {:} ",
				repository_id.clone(),
				issue_number.clone()
			);
			error!(error = e.to_field(), "{error_message}");
