...
pub async fn fetch_issue(docker: &'static Cli) {
	...
	test.should_fetch_an_issue_given_a_repo_owner_repo_name_issue_number_and_pat()
		.await
		.expect("should_fetch_an_issue_given_a_repo_owner_repo_name_issue_number_and_pat");
}
...
impl<'a> Test<'a> {
	async fn should_fetch_an_issue_given_a_repo_owner_repo_name_issue_number_and_pat(
		&mut self,
	) -> Result<()> {
