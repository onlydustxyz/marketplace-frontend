		...
		let mut connection = self.context.database.client.connection()?;
		{
...

			{
				...
				assert_eq!(
					...
					Some(json!({"base_sha": pr_1144().base_sha, "head_sha": pr_1144().head_sha}))
				);
			}
			{
				let state = states.pop().unwrap();
				...
				assert_eq!(
					state.pull_request_indexer_state,
					Some(json!({"base_sha": pr_1146().base_sha, "head_sha": pr_1146().head_sha}))
				);
			}
			{
				let state = states.pop().unwrap();
				...
				assert_eq!(
					state.pull_request_indexer_state,
					Some(json!({"base_sha": pr_1152().base_sha, "head_sha": pr_1152().head_sha}))
				);
			}
		}
